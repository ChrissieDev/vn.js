...
export default class VNActorElement extends HTMLElement {
    ...
    async connectedCallback() {
        const id = this.getAttribute("uid") || this.getAttribute("name") || "anonymous";
        ...
        if (isDefinition) {
            ...
            this.#initializationPromise
                ...
                .catch(error => {
                    console.error(`[INST ${id}] ASYNC Initialization error:`, error);
    ...
    attributeChangedCallback(name, oldValue, newValue) {
         if (oldValue !== newValue) {
            ...
            if (name === "name") { this.#name = newValue ?? ""; }
                 ...
                 console.warn(`Actor instance UID changed dynamically. May require manual re-initialization or state update.`);
            }
        }
    }
    ...
    #setupInstanceObserver() {
        ...
        const id = this.getAttribute("uid") || this.getAttribute("name") || "anonymous_inst";
        ...
        this.#instanceObserver = new MutationObserver(mutations => {
            mutations.forEach(mutation => {
                if (mutation.type === 'attributes' && mutation.attributeName.startsWith('&')) {
                    ...
                    const newStateUid = this.getAttribute(mutation.attributeName);
                     if (newStateUid !== null && newStateUid !== undefined) {
                        this.setState(bodyPartUid, newStateUid);
                     } else {
                         console.warn(`[INST ${id}] &-attribute ${mutation.attributeName} removed. State handling not implemented.`);
                     }
                }
            });
        });
    ...
    #parseDefinition() {
        ...
        const id = this.getAttribute("name") || this.getAttribute('uid') || "anonymous_def";
            ...
            for (const imgDef of stateDefs) {
                ...
                if (src) {
                    ...
                    imagePromises.push(
                        img.decode()
                            .then(() => {
                                this.#maxImageWidth = Math.max(this.#maxImageWidth, img.naturalWidth);
                                this.#maxImageHeight = Math.max(this.#maxImageHeight, img.naturalHeight);
                            })
                            .catch(e => console.warn(`[DEF ${id}] Failed decode image ${src}:`, e))
                    );
                }
            }
        }
        ...
        Promise.allSettled(imagePromises).then(() => {
            if (this.#maxImageWidth > 0 && this.#maxImageHeight > 0) {
                const aspectRatio = this.#maxImageWidth / this.#maxImageHeight;
                this.#calculatedRelativeHeight = (this.#maxImageHeight / DESIGN_HEIGHT) * 100;
                this.style.setProperty('--vn-actor-aspect-ratio', aspectRatio);
                this.style.setProperty('--vn-actor-calculated-height', `${this.#calculatedRelativeHeight.toFixed(2)}%`);
            } else {
                console.warn(`[DEF ${id}] Could not determine dimensions. Using defaults.`);
            ...
            console.error(`[DEF ${id}] Error during image dimension calculation:`, error);
            this.#isDefinitionParsed = true;
            this.style.setProperty('--vn-actor-aspect-ratio', '1 / 1');
            this.style.setProperty('--vn-actor-calculated-height', `60%`);
            rejectParse(error);
    ...
    #applyDynamicStyles() {
         const id = this.getAttribute("uid") || this.getAttribute("name") || "anonymous_inst";
         const hasInlineHeight = this.style.height || (this.getAttribute('style') || '').includes('height:');
         const hasInlineAspectRatio = this.style.aspectRatio || (this.getAttribute('style') || '').includes('aspect-ratio:');

         if (!hasInlineHeight) {
             const calculatedHeight = this.style.getPropertyValue('--vn-actor-calculated-height').trim();
         ...
         if (!hasInlineAspectRatio) {
             const calculatedAspectRatio = this.style.getPropertyValue('--vn-actor-aspect-ratio').trim();
    ...
    async #initializeFromAssets() {
        ...
        if (!this.isConnected) {
             console.warn(`[${this.id || 'unknown'}] Initialization called before connection. Awaiting connection.`);
             await new Promise(resolve => this.addEventListener('connected', resolve, {once: true}));
        }
        await this.#playerPromise;

        const uid = this.getAttribute("uid");
        if (!uid) { throw new Error("Actor instance requires a 'uid' attribute."); }
        ...
        if (!definition || !(definition instanceof VNActorElement)) {
             throw new Error(`[${instanceId}] Definition not found or invalid for uid "${uid}".`);
        }

        try {
            await definition.ensureParsedAsync();
        } catch (error) {
            console.error(`[${instanceId}] Error waiting for definition parsing:`, error);
        ...
        const aspectRatioVar = definition.style.getPropertyValue('--vn-actor-aspect-ratio').trim();
        const heightVar = definition.style.getPropertyValue('--vn-actor-calculated-height').trim();
        if (aspectRatioVar) this.style.setProperty('--vn-actor-aspect-ratio', aspectRatioVar);
        if (heightVar) this.style.setProperty('--vn-actor-calculated-height', heightVar);
            ...
            if (attr.name.startsWith("&")) {
                ...
                if (bpDef?.getImageDefinition(stateUid)) {
                    this.#activeState.set(bpUid, stateUid);
                     ...
                     console.warn(`[${instanceId}] Instance &-attribute "${attr.name}='${stateUid}'" is invalid for body part "${bpUid}". Using definition default or no state.`);
                 }
            }
        }
        ...
        const wrapper = this.#shadow.querySelector(".actor-wrapper");
        if (!wrapper) throw new Error(`[${instanceId}] CRITICAL: .actor-wrapper not found!`);
        wrapper.innerHTML = "";
    ...
    #addOrReplaceBodyPartImage(bodyPartUid, stateUid) {
        ...
        const bodyPartDefElement = definition?.getDefinitionBodyPartDefs()?.get(bodyPartUid);
        const imgDef = bodyPartDefElement?.getImageDefinition(stateUid);

        if (!imgDef) {
            console.error(`[${instanceId}] #addOrReplace: Image definition not found for ${bodyPartUid}/${stateUid}.`);
             const existingImg = wrapper.querySelector(`img[data-body-part="${bodyPartUid}"]`);
             if (existingImg) {
                 console.warn(`[${instanceId}] #addOrReplace: Removing image for ${bodyPartUid} due to invalid new state ${stateUid}.`);
                 existingImg.remove();
             }
            return false;
        }
        ...
        const zIndex = imgDef.style.zIndex || imgDef.getAttribute("z-index") || "auto";
        newImgInstance.style.zIndex = zIndex;
        const imgStyle = imgDef.getAttribute("style") || "";
         if (imgStyle) {
            const currentInlineStyle = newImgInstance.getAttribute('style') || '';
            newImgInstance.setAttribute("style", `${imgStyle}; ${currentInlineStyle}`);
         }

        newImgInstance.onerror = () => { console.error(`[${instanceId}] Failed load image: ${src}`); newImgInstance.style.display = 'none'; };
    ...
    setState(bodyPartUid, stateUid) {
        ...
        if (!isInstance) {
            ...
            } else { console.warn(`${logPrefix} setState (Default): Invalid state "${stateUid}" for body part "${bodyPartUid}".`); }
            return;
        }

         this.ensureInitialized().then(() => {
             if (!this.#isInstanceInitialized) {
                  console.warn(`${logPrefix} setState: Instance failed to initialize. Ignoring ${bodyPartUid}=${stateUid}.`);
                  return;
             }
             ...
             const success = this.#addOrReplaceBodyPartImage(bodyPartUid, stateUid);

             if (success) {
                 this.#activeState.set(bodyPartUid, stateUid);
             } else {
                  console.warn(`${logPrefix} setState: Failed to set state for ${bodyPartUid} to ${stateUid}.`);
                  if (!this.#shadow.querySelector(`.actor-wrapper img[data-body-part="${bodyPartUid}"]`)) {
                       this.#activeState.delete(bodyPartUid);
                  }
             }

         }).catch(error => {
              console.error(`${logPrefix} setState: Error during initialization wait:`, error);
         });
    }
    ...
    getName() { return this.getAttribute("name") || this.#name || this.getAttribute('uid') || ""; }
    getDefaultActiveState() {
        if (this.closest('vn-assets') && !this.#isDefinitionParsed) {
            console.warn(`[DEF ${this.id}] getDefaultActiveState: Definition not parsed yet.`);
        }
    ...
    getDefinitionBodyPartDefs() {
         if (this.closest('vn-assets') && !this.#isDefinitionParsed) {
             console.warn(`[DEF ${this.id}] getDefinitionBodyPartDefs: Definition not parsed yet.`);
         }
         return this.#definitionBodyPartDefs;
    }


    isParsed() { return this.closest("vn-assets") !== null && this.#isDefinitionParsed; }
    get isInitialized() { return !this.closest("vn-assets") && this.#isInstanceInitialized; }
    getMaxImageWidth() {
         if (this.closest('vn-assets') && !this.#isDefinitionParsed) { console.warn(`[DEF ${this.id}] getMaxImageWidth: Definition not parsed yet.`); }
         return this.#maxImageWidth;
    }
    getMaxImageHeight() {
         if (this.closest('vn-assets') && !this.#isDefinitionParsed) { console.warn(`[DEF ${this.id}] getMaxImageHeight: Definition not parsed yet.`); }
         return this.#maxImageHeight;
    }
    getCalculatedRelativeHeight() {
         if (this.closest('vn-assets') && !this.#isDefinitionParsed) { console.warn(`[DEF ${this.id}] getCalculatedRelativeHeight: Definition not parsed yet.`); }
    ...
    ensureInitialized() {
         ...
         if (!this.#initializationPromise) {
              ...
              this.#initializationPromise.catch(error => {
                console.error(`[INST ${this.id}] ASYNC Init error (ensureInitialized):`, error);
                this.style.setProperty('display', 'none', 'important');
              });
         }
    ...
    async forceInitialize() {
       ...
       if (uid && !this.closest('vn-assets')) {
            ...
            catch (e) { console.error(`[INST ${uid}] forceInitialize error:`, e); this.style.setProperty('display', 'none', 'important'); }
       }
    }
}
