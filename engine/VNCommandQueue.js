...
export default class VNCommandQueue {
    ...
    constructor({ player, parentQueue = null, commands = [], i = 0 } = {}, ...initialCommands) {
        if (!player || !(player instanceof VNPlayerElement)) {
            throw new Error("VNCommandQueue requires a valid VNPlayerElement instance.");
        }
        this.player = player;
        ...
        const sourceCommands = initialCommands.length > 0 ? initialCommands : commands;

        if (sourceCommands.length > 0) {
            this.commands = initialCommands.length > 0 ? this.parseCommands(...sourceCommands) : sourceCommands;
    ...
    parseCommands(...commandsToParse) {
        ...
        for (let idx = 0; idx < commandsToParse.length; idx++) {
            ...
            if (typeof currentItem === 'string') {
                const lastAddedCommand = result.length > 0 ? result[result.length - 1] : null;
                let speakerUid = '';
                let speakerName = '';

                if (lastAddedCommand instanceof VNCommandSay && lastAddedCommand.actorUid !== 'you') {
                    speakerUid = lastAddedCommand.actorUid;
                    speakerName = lastAddedCommand.actorName;
                }
                parsedCommand = new VNCommandSay(this, speakerUid, speakerName, currentItem);

            } else if (currentItem instanceof VNCommand || currentItem instanceof VNCommandQueue) {
                 parsedCommand = currentItem;
                 if (parsedCommand instanceof VNCommand && !parsedCommand.queue) {
                     parsedCommand.queue = this;
                 }
                 if (parsedCommand instanceof VNCommandSetActorState && !parsedCommand.queue) {
                      parsedCommand.queue = this;
                 }

            } else if (typeof currentItem === 'object' && currentItem !== null && currentItem.type) {
                 parsedCommand = this.parseApiObject(currentItem);

            } else if (typeof currentItem === 'function') {
                 console.warn("VNQ Parse: Bare function found. Wrap in $() or ensure it returns a command.", currentItem);
                 parsedCommand = null;
            ...
            if (parsedCommand) {
                ...
                if (parsedCommand instanceof VNCommandElse) {
                    if (lastIfCommand) {
                        if (!parsedCommand.attachToIf(lastIfCommand)) { console.error("VNQ Parse: Failed ELSE attach."); }
                    } else { console.error("VNQ Parse: ELSE without IF."); }
                    continue;
                }
    ...
    parseApiObject(commandObject) {
        switch (commandObject.type) {
            case 'say':
                if (typeof commandObject.actorUid !== 'string' || typeof commandObject.actorName !== 'string' || typeof commandObject.text !== 'string') {
                    console.error("VNQ Parse Error: Invalid 'say' object structure.", commandObject); return null;
                }
                return new VNCommandSay(this, commandObject.actorUid, commandObject.actorName, commandObject.text, !!commandObject.isMonologue);
            case 'add':
                 if (typeof commandObject.objectType !== 'string' || typeof commandObject.uid !== 'string') {
                     console.error("VNQ Parse Error: Invalid 'add' object structure.", commandObject); return null;
                 }
                return new VNCommandAddObject(this, commandObject.objectType, commandObject.uid, commandObject.options || {});
            case 'start':
    ...
    executeCurrent() {
        ...
        if (!command || !(command instanceof VNCommand)) {
            console.error(`VNQ executeCurrent: Invalid or missing command at index ${this.i}. Skipping.`, command);
            this.i++;
            return true;
        }
        ...
        try {
            ...
            console.error(`%cError executing command [${this.i}] ${command.constructor.name}:`, 'color: red; font-weight: bold;', error, command);
    ...
    set scene(value) {
        if (value && value instanceof VNSceneElement) {
            ...
            if (this.#scene !== null) {
                 this.#scene = null;
                 this.commands.forEach(cmd => { if (cmd instanceof VNCommand) cmd.scene = null; });
            }
        } else {
